...
A typical Seahorse run includes three measurement cycles for each phase. 
...
This can limit not only the interpretation of Seahorse XF OCR outcomes, but also the development of mitochondrial targeted therapies as well as our understanding of mitochondrial biology. 
...
A typical Seahorse run contains four intervals. 
...
In a typical Seahorse XF assay, the same cell line from the same experimental group is seeded in multiple wells. 
...
The Seahorse XF assay for a (patient) cell line or a specific condition is often repeated on different days and on more than one plates. 
...
We observed that both *BOLA3* and *PET100* patient cell line showed reduction in ATP-linked respiration, basal respiration and maximal respiration, compared to the control cell line. 
On the other hand, these two patient cell lines showed no difference in proton leak and spare respiratory capacity compared to the control cell line (Figure 1D and 1H). 
...
To demonstrate that OCRbayes works properly, we applied it to analyze the published Seahorse XF OCR data set containing 26 patient cell lines as well as a control cell line. 
This data set contains 6 cell lines were labeled as positive controls and 2 cell lines were labeled as negative controls (as explained in the material and methods section). 

Based on our analysis, we found 6 patient cell lines that had lower maximal respiration compared to the control cell line with False Discovery Rate (FDR) below 0.05 (Figure 3B). 
...
Five of these 6 patient cell lines were actually the positive controls. 
...
Our separate analysis showed that only the second experiment showed difference in maximal respiration (Figure 4B). 
When we used OCRbayes to analyze all the three plates together, we found that a tendency that the patient cell line had lower maximal respiration than the control cell line. 
However, this tendency was not statistically significant. 
...
One patient cell line was was the *PET100* gene mutation fibroblast (patient cell line 73387) and the other patient cell line was *NSUN3* gene mutation fibroblast (patient cell line 76065). 
...
We suggest that whether performing a single or multiple Seahorse assay depends on the effect size of the perturbation. 
OCRbayes allows us to calculate posterior probability that the maximal respiration (or any other respiration metrics) was abnormal in a patient even based on a single Seahorse assay. 
